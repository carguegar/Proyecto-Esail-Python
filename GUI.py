...
def interfaz_bienvenida():
    ...
    imagen_original = Image.open("c:/Users/carlo/Desktop/python/fondo.png")
    ...
    boton_salir = Button(ventana, text="Salir", font=("Arial", 14), command=cerrar_programa, bg="lightgreen")
...
def mostrar_menu_principal(carrito, cliente):
    ...
    boton_cerrar_sesion = Button(ventana, text="Cerrar Sesión", font=("Arial", 14),
                                 command=cerrar_sesion, bg="lightgreen")
...
def mostrar_cesta_compra(cesta_compra, cliente):
    ...
    Button(ventana, text="Regresar al Menú Principal", font=("Arial", 14), bg="lightblue",
           command=lambda: boton_regresar_a_menu_cliente(cesta_compra, cliente)).grid(row=len(cesta_compra[0]) + 2, column=0, columnspan=2, pady=20)
...
def mostrar_cesta_vacia(ventana):
    Label(ventana, text="La cesta de compra está vacía.", font=("Arial", 16), bg="lightblue").grid(row=1, column=0, columnspan=2, pady=20)
...
def mostrar_producto_en_carrito(ventana, producto, cantidad, indice, cesta_compra, cliente):
    ...
    frame_producto.grid(row=indice + 1, column=0, columnspan=2, padx=20, pady=10, sticky="ew")

    # Información del producto
    texto_producto = f"{producto.nombre_producto} - Precio Unitario: {producto.precio}€ - Precio Total: {producto.precio * cantidad}€"
    Label(frame_producto, text=texto_producto, font=("Arial", 14), bg="lightblue").grid(row=0, column=0, padx=10, pady=5, sticky="w")

    # Controles para modificar la cantidad
    frame_controles = Frame(frame_producto, bg="lightblue")
    frame_controles.grid(row=1, column=0, padx=10, pady=5, sticky="e")
...
def mostrar_tickets(carrito,cliente):
    """
    ...
    if tickets_cliente:
        ...
        encabezado_producto.grid(row=1, column=0, padx=10, pady=5, sticky="w")
        ...
        for i, ticket in enumerate(tickets_cliente, start=2):
            ...
            etiqueta_producto.grid(row=i, column=0, padx=10, pady=5, sticky="w")
    ...
    # Botón para regresar al menú principal
    Button(ventana, text="Regresar al Menú Principal", font=("Arial", 14), bg="lightblue",
           command=lambda: boton_regresar_a_menu_cliente(carrito,cliente)).grid(row=i+2, column=0, columnspan=5, pady=20)
...
def mostrar_datos_cliente(carrito,cliente):
    ...
    boton_modificar_tarjeta = Button(ventana, text="Modificar", font=("Arial", 12), bg="lightgreen", command=lambda: modificar_numero_tarjeta(carrito,cliente))
    ...
    boton_modificar_tlf = Button(ventana, text="Modificar", font=("Arial", 12), bg="lightgreen", command=lambda: modificar_numero_tlf(carrito,cliente))
    boton_modificar_tlf.grid(row=6, column=1, padx=10, pady=5)

    # Contraseña con botón para modificar
    boton_modificar_contrasena = Button(ventana, text="Cambiar contraseña", font=("Arial", 12), bg="lightgreen", command=lambda: modificar_contrasena(carrito,cliente))
...
def guardar_cambio(carrito,cliente,nuevo_numero,atributo):
        if nuevo_numero:
...
def modificar_numero_tarjeta(carrito,cliente):
    ...
    entrada_numero_tarjeta.pack(pady=10)
    Button(ventana, text="Guardar", font=("Arial", 14), bg="lightgreen", command=lambda: guardar_cambio(carrito,cliente,entrada_numero_tarjeta.get().strip(),"numero_tarjeta")).pack(pady=10)
...
def modificar_numero_tlf(carrito,cliente):
    ...
    entrada_numero_tlf.pack(pady=10)
    Button(ventana, text="Guardar", font=("Arial", 14), bg="lightgreen", command=lambda:guardar_cambio(carrito,cliente,entrada_numero_tlf.get().strip() ,"numero_tlf")).pack(pady=10)
    # Botón para regresar a los datos del cliente
    Button(ventana, text="Regresar", font=("Arial", 14), bg="lightblue", command=lambda: actualizar_ventana(lambda: mostrar_datos_cliente(carrito,cliente))).pack(pady=10)
...
def modificar_contrasena(carrito, cliente):
    ...
    Button(ventana, text="Guardar", font=("Arial", 14), bg="lightgreen", command=lambda: guardar_cambio_contrasena(entrada_contrasena.get(),entrada_nueva_contrasena.get(),entrada_repetir_contrasena.get() ,carrito, cliente)).pack(pady=10)
    # Botón para regresar a los datos del cliente
    Button(ventana, text="Regresar", font=("Arial", 14), bg="lightblue", command=lambda: actualizar_ventana(lambda: mostrar_datos_cliente(carrito,cliente))).pack(pady=10)
